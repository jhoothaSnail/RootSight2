...
import { ArrowRight, Mail, Lock, Eye, EyeOff, Network, ShieldCheck, Sparkles, Check } from 'lucide-react';
...
export default function Login({ onNavigate }: LoginProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // NOTE: Mocked auth for now — no backend auth endpoint wired yet.
    // Replace with a real POST /api/auth/login call once the backend contract exists.
    ...
    <div className="flex-1 w-full flex overflow-hidden bg-zinc-950 selection:bg-amber-500/30">
      ...
      <div className="w-full lg:w-[46%] relative z-10 flex flex-col px-8 sm:px-14 py-10 overflow-y-auto">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-sm w-full mx-auto">
          ...
          >
            ...
            <p className="text-center text-sm text-zinc-500 mt-2">
              ...
              <button
                type="button"
                onClick={() => {
                  /* Wired to a real Sign Up page in the next phase */
                }}
          ...
          <button
            ...
            className="group inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800 hover:border-teal-500/40 text-zinc-300 hover:text-white font-semibold uppercase tracking-wider text-xs transition-all backdrop-blur-sm"
          >
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden border-l border-zinc-900">
        ...
        <div className="relative z-10 flex flex-col justify-center px-16 max-w-xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-zinc-900/80 border border-zinc-800 rounded font-mono text-xs text-zinc-400 uppercase tracking-widest mb-8 backdrop-blur-sm w-fit">
          ...
          <p className="text-zinc-400 font-light leading-relaxed text-base xl:text-lg">
            Your architecture maps, root cause analyses, and runbooks are saved automatically —
