        ...
        {!file && status === 'idle' && (
          <div className="flex items-center gap-3">
             ...
             <div>
               ...
               <p className="text-[10px] text-zinc-600 font-mono mt-0.5 uppercase tracking-wider">PDF, DOCX, TXT, MD, CSV, XLSX</p>
             </div>
          </div>
        )}
...
// Maps the UI source slots to the finalized /api/upload contract field names.
const FIELD_MAP: Record<string, string> = {
  'arch-doc': 'architectureDoc',
  'srv-cat': 'serviceCatalog',
  'inc-hist': 'incidentHistory',
};
...
export default function IntelligenceWorkspace({ onComplete }: IntelligenceWorkspaceProps) {
  ...
  const [optionalFiles, setOptionalFiles] = useState<{id: string, file: File}[]>([]);
  ...
  const handleUploadRequired = (id: string, file: File, label: string) => {
    setRequiredFiles(prev => ({ ...prev, [id]: file }));
    addFeedItem(`${label} Uploaded: ${file.name}`);
  };
  ...
  const handleUploadOptional = (file: File) => {
    const newId = Math.random().toString();
    setOptionalFiles(prev => [...prev, { id: newId, file }]);
    addFeedItem(`Additional Source Uploaded: ${file.name}`);
  };
  ...
  const startExtraction = async () => {
    setErrorMsg(null);
    setPipelineStage(1);
    setActiveOperation('Uploading Documents');
    addFeedItem('Started Intelligence Extraction');

    const files: UploadFile[] = [];
    const slots: Array<[string, File | null]> = Object.entries(requiredFiles);
    for (const [id, f] of slots) {
      if (f) files.push({ field: FIELD_MAP[id] || id, file: f });
    }
    for (const o of optionalFiles) {
      files.push({ field: 'additionalDoc', file: o.file });
    }
    ...
    try {
      ...
      addFeedItem(`${s.services} Services Identified`);
      await sleep(700);
      setActiveOperation('Extracting Teams & Vendors');
      addFeedItem(`${s.teams} Teams Identified`);
      addFeedItem(`${s.vendors} Vendors Identified`);
      addFeedItem(`${s.databases} Databases Identified`);
      await sleep(700);

      setPipelineStage(2);
      setActiveOperation('Classifying Relationships');
      addFeedItem(`${s.relationships} Relationships Classified`);
      await sleep(700);

      setPipelineStage(3);
      setActiveOperation('Ready for Graph Generation');
    } catch (err) {
      const msg = err instanceof ApiClientError ? err.message : 'Extraction failed. Please try again.';
      addFeedItem(`Error: ${msg}`);
      setErrorMsg(msg);
      setActiveOperation('Extraction Failed');
      setPipelineStage(0);
    }
  };
  ...
  const nodesCreated = summary
    ? summary.services + summary.vendors + summary.teams + summary.databases + summary.engineers + summary.incidents + summary.runbooks
    : 0;
                    ...
                    {requiredSourcesList.map(src => (
                      <div key={src.id}>
                        <FileUploader 
                          id={src.id}
                          title={src.label}
                          file={requiredFiles[src.id]?.name ?? null}
                          disabled={pipelineStage > 0}
                          onUpload={(f) => handleUploadRequired(src.id, f, src.label)}
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-4">
                        ...
                        <div className="flex items-center justify-between w-full">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <p className="text-sm font-medium text-zinc-200 truncate max-w-[150px] sm:max-w-[300px]">{file.file.name}</p>
                            </div>
                          </div>
                          {pipelineStage === 0 && (
                            <button 
                    ...
                    <FileUploader 
                      ...
                      disabled={pipelineStage > 0}
                      isOptional={true}
                      onUpload={(f) => handleUploadOptional(f)}
                      onRemove={() => {}}
                    />
                  </div>
                </div>
              ...
              <div className="shrink-0 pt-4 border-t border-zinc-800 flex flex-col items-end gap-2">
                {pipelineStage === 0 && (
                  <>
                    ...
                    {errorMsg ? (
                      <p className="text-[10px] text-red-400 font-mono flex items-center gap-1.5"><AlertTriangle className="w-3 h-3 text-red-500" /> {errorMsg}</p>
                    ) : !isRequiredComplete ? (
                ...
                >
                  ...
                  <div className="flex gap-6 font-mono text-xs">
                     <div className="flex flex-col items-end">
                       ...
                       <span className="text-teal-400 font-bold text-sm">{nodesCreated}</span>
                     </div>
                     <div className="flex flex-col items-end">
                       <span className="text-teal-500/70 uppercase tracking-widest">Relationships</span>
                       <span className="text-teal-400 font-bold text-sm">{summary?.relationships ?? 0}</span>
                     </div>
                  </div>
                </motion.div>
              )}
